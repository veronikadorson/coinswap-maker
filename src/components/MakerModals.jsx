...
import { StatusDot } from './MakerComponents.jsx';

// ─── AddMakerModal ────────────────────────────────────────────────────────────
export function AddMakerModal({ open, onClose, onCreate, nextIndex }) {
  const [name, setName] = useState("");
  const [port, setPort] = useState(6112);
  const [fidelity, setFidelity] = useState(0.00010000);
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="field">
            ...
            <div className="hint"><span>Used in your dashboard and logs.</span><span></span></div>
          </div>
          <div className="field">
            ...
            <div className="hint"><span>Must be free locally.</span><span>6100–6999</span></div>
          </div>
          <div className="field">
            ...
            <div className="hint"><span>Locked into a timelocked contract to prove staking.</span><span>BTC</span></div>
          </div>
        </div>
...
// ─── ManageDrawer ─────────────────────────────────────────────────────────────
export function ManageDrawer({ maker, open, onClose, onStart, onStop, onRestart, onDelete }) {
  useEffect(() => {
    if (!open) return;
    const onKey = (e) => { if (e.key === "Escape") onClose(); };
    window.addEventListener("keydown", onKey);
    ...
    <>
      ...
      <div className={`drawer ${open ? "open" : ""}`}>
          ...
          <div>
            ...
            <div className="metric-row">
              ...
              <div className="metric-cell">
                ...
                <div className="v">{fmtBtc(maker.balances.regular)} <small>BTC</small></div>
              </div>
              <div className="metric-cell">
                <div className="lbl">Swap</div>
                <div className="v">{fmtBtc(maker.balances.swap)} <small>BTC</small></div>
              </div>
              <div className="metric-cell">
          ...
          <div>
            ...
            <div className="metric-row">
              ...
              <div className="metric-cell">
                ...
                <div className="v" style={{ color: "var(--green)" }}>+{fmtSats(maker.earnedSats)}</div>
              </div>
              <div className="metric-cell">
        ...
        <div className="drawer-foot">
          ...
          ) : maker.status === "warming" ? (
            <button className="btn ghost" onClick={() => onStop(maker)} style={{ flex: 1 }}>
              <span className="icn">{ICONS.power}</span>
              Stop
            </button>
          ) : (
