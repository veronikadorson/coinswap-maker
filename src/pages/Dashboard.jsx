...
import { ICONS, MAKERS_BASE, ACTIVITY, fmtBtc, fmtSats } from '../data.jsx';
import { MetricCard, MakerCard, FilterTabs, ActivityCard } from '../components/MakerComponents.jsx';
import { AddMakerModal, ManageDrawer } from '../components/MakerModals.jsx';
import {
...
export default function Dashboard() {
  ...
  // Pick up new maker from Add Maker page via localStorage
  useEffect(() => {
    try {
      ...
      setTimeout(() => {
        setMakers((prev) => prev.map((x) => x.id === m.id
          ? { ...x, status: "running", uptime: "00:01:08" } : x));
      }, 1800);
    } catch (_) {}
  }, []);
  ...
  const [drawerMaker, setDrawerMaker] = useState(null);
  ...
  const openManage = (m) => { setDrawerMaker(m); setDrawerOpen(true); };
  const closeDrawer = () => {
    setDrawerOpen(false);
  ...
  return (
    <div className="page">
      ...
      <div className="section-head">
        ...
        <button className="btn" onClick={() => navigate('/add-maker')}>
