...
export default function AddMaker() {
  ...
  const [pcState, setPcState] = useState(() =>
    Object.fromEntries(PRECHECKS.map((r) => [r.id, "idle"]))
  );
  ...
  const isDirty = useMemo(() =>
    Object.keys(DEFAULTS).some((k) => form[k] !== DEFAULTS[k]),
  [form]);

  const pcCounts = useMemo(() => {
  ...
  const runAll = () => {
    PRECHECKS.forEach((row) => setPcState((p) => ({ ...p, [row.id]: "idle" })));
    PRECHECKS.forEach((row, i) => setTimeout(() => runCheck(row.id), 220 * i));
  };
        ...
        <section className="section-card">
          ...
          <div className="precheck-foot">
            <span className="summary">
              ...
              {pcCounts.running > 0 && <span>Running…</span>}
            </span>
